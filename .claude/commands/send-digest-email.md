...
每日截圖 Daily AI Digest 網頁，並透過 Render server 以 Gmail 寄出精美 HTML 電子報。
...
（Gmail 憑證 `GMAIL_USER` / `GMAIL_APP_PASSWORD` 設定在 Render 環境變數，不需在本地設定）
...
- 步驟 4 輸出 `✓ Email sent to jjaass9507@gmail.com`（或 EMAIL_TO 設定的地址）
